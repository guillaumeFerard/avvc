...
These elements are described in different folders. All the modelling happens within the `openfisca_avvc` folder.
...
```sh
git clone git://github.com/guillaumeFerard/avvc.git.git
cd avvc.git
...
```sh
openfisca serve --port 5000 --country-package openfisca_avvc
```
...
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_avvc/situation_examples/couple.json \
